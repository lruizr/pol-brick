...
def getFilteredComponents_stable(sort, order, value, limit, userId):
  ...
  if sort == "stars":
    if order == "asc":
      results = Repo.query().order(Repo.stars).fetch(limit)
    else:
      results = Repo.query().order(-Repo.stars).fetch(limit)
  elif sort == "popular":
    if order == "asc":
      results = Repo.query().order(Repo.reputation).fetch(limit)
    else:
      results = Repo.query().order(-Repo.reputation).fetch(limit)
  elif sort == "name":
    if order == "asc":
      results = Repo.query().order(Repo.name_repo_lower_case).fetch(limit)
    else:
      results = Repo.query().order(-Repo.name_repo_lower_case).fetch(limit)

  # Ckeck if there are results
  if not results == None:
    for item in results:
      componentMessage = item.toRPCMessage('basic')
      if not userId == 0:
        userRating = getUserRating(item.full_name_id, userId)
        componentMessage.userRating = userRating
      components.append(componentMessage)
...
def getFilteredComponents(sort, order, value):
  ...
  if sort == "stars":
    if order == "asc":
      res = Repo.query() #.order(Repo.stars)
      qry = Repo.query(ndb.OR(ndb.AND(Repo.name_repo_lower_case >= str(value), Repo.name_repo_lower_case <= str(value) + "z")))#,
                              #ndb.AND(Repo.name_repo_lower_case >= value, Repo.name_repo_lower_case <= "z" +  value)))

      results = qry.fetch(100)
    else:
      res = Repo.query() #.order(-Repo.stars)
      qry = Repo.query(ndb.OR(ndb.AND(Repo.name_repo_lower_case >= str(value), Repo.name_repo_lower_case <= str(value) + "z")))#,
                              #ndb.AND(Repo.name_repo_lower_case >= value, Repo.name_repo_lower_case <= "z" + value)))
    ...
    if order == "asc":
      res = Repo.query() #.order(Repo.reputation)
      qry = Repo.query(ndb.OR(ndb.AND(Repo.name_repo_lower_case >= str(value), Repo.name_repo_lower_case <= str(value) + "z")))#,
                              #ndb.AND(Repo.name_repo_lower_case >= value, Repo.name_repo_lower_case <= "z" +  value)))
      results = qry.fetch(100)
    else:
    ...
    if order == "asc":
      res = Repo.query() #.order(Repo.name_repo_lower_case)
      qry = Repo.query(ndb.OR(ndb.AND(Repo.name_repo_lower_case >= str(value), Repo.name_repo_lower_case <= str(value) + "z")))#,
                              #ndb.AND(Repo.name_repo_lower_case >= value, Repo.name_repo_lower_case <= "z" +  value)))
      results = qry.fetch(100)
    else:
  ...
  if not results == None:
    for item in results:
      components.append(item.toRPCMessage('basic'))
